# LeadFlow — Sales Lead Management Platform

LeadFlow is a high-performance, full-stack lead management platform built with the MERN stack (MongoDB, Express, React, Node.js) and TypeScript. Meticulously designed for sales teams, marketing managers, and executives, the application enables companies to track their customer acquisition pipeline, monitor conversion performance metrics via live KPI dashboard panels, assign follow-ups, and export historical CSV records. It implements "Obsidian UI" — a custom-engineered, dark-first premium design system using Tailwind CSS and CSS variables.
...
| **Schema Validation** | `Zod` (Shared shared package validations) | `^ 3.23.8` |
...
*   **⚡ Reactive Filtering & Debouncing**: Live full-text search with a 400ms pulsing loading border, dropdown status/source filters, and pagination controllers.
...
*   **🛡️ Role-Based Access Control**: Gates actions dynamically. Only administrators can delete any lead or manage user records. Sales/Managers can create/edit leads and trigger data downloads.
...
### 🐳 With Docker (Recommended — Instant Setup in Under 3 Minutes)
...
4.  **Seed Custom Pipeline Data (Optional)**:
    While containers are running, execute the idempotent seed command inside a second terminal window to populate the pipeline:
    ```bash
...
4.  **Start All Services Concurrently**:
    ...
    *   The **Backend Server** on [http://localhost:5000](http://localhost:5000) (with hot reload).
...
*   **🛡️ System Administrator** *(Full pipeline CRUD, system-wide User Management, and Delete Rights)*:
